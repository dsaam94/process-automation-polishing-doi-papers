...
fs.readFile( 'main_data/proceedings/edbt.xml', function(err, data) {
	if(data){
		...
		//Tester
		//console.log(json.conf.track[1].paper[0]);
		...
		if(json && json.conf && json.conf.track){
			...
			let meta_data_results = check_metadata(combined_json_papers);
			...
			if(meta_data_results.success){
				...
				//throw new Error("Error: " + meta_data_results.message);
			}
	    }else{
			  ...
			  papers.forEach((e,i) => {
						...
						if(e['author'] && e['title'] && e['path'] 
							...
							if(e['author'].length || Object.keys(e['author']).length){
								try {
								  if (!fs.existsSync('main_data/proceedings/' + e['pitch']['$t'])) {
								    //file doesnot exists
								  ...
								  if (!fs.existsSync('main_data/proceedings/' + e['poster']['$t'])) {
								  ...
								  if (!fs.existsSync('main_data/proceedings/' + e['path']['$t'])) {
								    //file does not exists
								...
								try {
								  ...
								  if (!check_file_pages_a4_result) { // Change Here Here Here
								    //file size exceeded
